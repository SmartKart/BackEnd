...
router.post('/api/v1/store', (req, res, next) => {
  const results = [];
  // Grab data from http request
  const data = {id: req.body.id, name: req.body.name, price: req.body.price, percentOff: req.body.percentOff, isOnSale: req.body.isOnSale, type:req.body.type };
  // Get a Postgres client from the connection pool
  pg.connect(connectionString, (err, client, done) => {
    // Handle connection errors
    if(err) {
    ...
    // SQL Query > Insert Data
    client.query('INSERT INTO store(id, name, price, percentOff, isOnSale, type) values($1, $2, $3, $4, $5, $6)',
    [data.id, data.name, data.price, data.percentOff, data.isOnSale, data.type]);
    // SQL Query > Select Data
...
router.put('/api/v1/store/:item_id', (req, res, next) => {
  ...
  // Grab data from http request
  const data = {name: req.body.name, price: req.body.price, percentOff: req.body.percentOff, isOnSale: req.body.isOnSale, type:req.body.type};
  // Get a Postgres client from the connection pool
  pg.connect(connectionString, (err, client, done) => {
    // Handle connection errors
    if(err) {
    ...
    // SQL Query > Update Data
    client.query('UPDATE store SET name=($1), price=($2), percentOff=($3), isOnSale=($4), type=($5) WHERE id=($6)',
    [data.name, data.price, data.percentOff, data.isOnSale, data.type, id]);
    // SQL Query > Select Data
    const query = client.query("SELECT * FROM store ORDER BY id ASC");
    // Stream results back one row at a time
    query.on('row', (row) => {
